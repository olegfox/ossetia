 ...
 *
 * @ORM\Table(name="news_paper")
 * @ORM\Entity(repositoryClass="Site\MainBundle\Entity\Repository\NewsPaperRepository")
 */
class NewsPaper
{
    ...
    public function getAbsolutePathDocument()
    {
        return null === $this->document
            ? null
            : $this->getUploadRootDirDocument().'/'.$this->document;
    }

    public function getWebPathDocument()
    {
        return null === $this->document
            ? null
            : $this->getUploadDirDocument().'/'.$this->document;
    }

    protected function getUploadRootDirDocument()
    {
        return __DIR__.'/../../../../../'.$this->getUploadDirDocument();
    }

    protected function getUploadDirDocument()
    {
    ...
    public function uploadDocument()
    {
        ...
        if (isset($this->document)) {
            if(file_exists($this->getUploadDirDocument().'/'.$this->document)){
                unlink($this->getUploadDirDocument().'/'.$this->document);
            }
            $this->document = null;
        }

        $this->document = $this->getFileDocument()->getClientOriginalName();

        $this->getFileDocument()->move(
            $this->getUploadDirDocument(),
            $this->document
        );
